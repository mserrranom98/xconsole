...
import { GetActionTables, NameStatus, GetActionUpdate, GetActionSection, GetUpdateSection } from '../../../models/desktop';
...
export class SectionmanagementComponent implements OnInit {
  ...
  constructor(
    ...
    { 'argument': '', 'value': '' },
    { 'argument': '', 'value': '' }]);
    this.upSec = new GetUpdateSection('', '', '', '', [
  ...
  onChange(data) {
    ...
    this.argUpdate.serviceArguments[1].value = 'USR$SECTION';
...
updatePages(rowIndex) {

  this.upSec.serviceName = 'UPDATE_SECTIONS';
    this.upSec.userToken = VARGLOBAL.userToken;
    this.upSec.sessionUUID = VARGLOBAL.sessionUUID;
    this.upSec.jsessionID = VARGLOBAL.jsessionID
    this.upSec.serviceArguments[0].argument = 'id';
    this.upSec.serviceArguments[0].value = this.data[rowIndex].id;
    this.upSec.serviceArguments[1].argument = 'enabled';
    this.upSec.serviceArguments[1].value = this.values;
    this.getAction.updateSection(this.upSec).subscribe(
      (response:any) => {
      }
    )
}
