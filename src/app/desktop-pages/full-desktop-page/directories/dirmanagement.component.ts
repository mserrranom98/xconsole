...
@Component({
  selector: 'app-dirmanagement',
  templateUrl: './dirmanagement.component.html'
})
export class DirmanagementComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getAction.tables(this.arg).subscribe(
      (response:any) => {
        this.data = response.directories;
        this.temp = [...this.data];
        this.rows = this.data;
      },
      error => {
    ...
    this.getAction.doUpdate(this.argUpdate).subscribe(
      (response:any) => {
        if (response.messageCode === '0') {
          ...
          this.fields = '';
          this.values = '';
          this.router.navigate(['/directories'], { relativeTo: this.route.parent });
        }
      },
      error => {
        console.log(<any>error);
  ...
  updateFilter(event) {
    const val = event.target.value.toLowerCase();
        ...
        const temp = this.temp.filter(function (d) {
            return d.DIRECTORY_NAME.toLowerCase().indexOf(val) !== -1 || !val;
        });
