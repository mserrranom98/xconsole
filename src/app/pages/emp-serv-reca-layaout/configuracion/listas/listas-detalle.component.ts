...
import { ListaIU, ListasItems, LItemIU, LItemDelete, Listas } from '../../../pages-models/model-emp-serv-rec';
...
export class ListasDetalleComponent implements OnInit {
  ...
  onNuevo() {
    ...
    this.listasService.insertLDet(this.litemIU).subscribe(
      (response:any) => {
        if (response.code === '0') {
  ...
  onEditar() {
    this.litemIU.lista = this.listasItems.lista;
    this.listasService.updateLDet(this.litemIU).subscribe(
      (response:any) => {
        if (response.code === '0') {
  ...
  eliminar(rowIndex) {
    ...
    swal({
      ...
      if (result.value) {
        this.listasService.deleteLDet(this.lItemDelete).subscribe(
          (response:any) => {
            if (response.code === '0') {
